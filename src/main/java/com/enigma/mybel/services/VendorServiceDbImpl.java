...
public class VendorServiceDbImpl implements VendorService{
    ...
    @Override
    public Vendor saveVendor(MultipartFile videos,MultipartFile file,String requestBody) throws IOException {
        ...
        if(videos!=null) vendor.setVideo(String.format(Constant.PATHVENDOR,fileUtil.upload(vendor.getId(),videos)));
    ...
    @Override
    public Vendor updateVendor(MultipartFile videos,MultipartFile file,String requestBody) throws IOException {
        Vendor vendor = repository.save(objectMapper.readValue(requestBody,Vendor.class));
